...
import { AGENT_COLORS, cn, getAgentColor } from "./utils";
import { formatCredits, formatCreditsCompact } from "./format";
...
describe("utils", () => {
  ...
  describe("formatCredits (canonical full-precision, from @/lib/format)", () => {
    test("formats millions with full precision and cr suffix", () => {
      const result = formatCredits(1_000_000);
      expect(result).toBe("1,000,000 cr");
    });

    test("formats thousands with full precision", () => {
      const result = formatCredits(1_000);
      expect(result).toBe("1,000 cr");
    });
  ...
  describe("formatCreditsCompact (canonical compact, from @/lib/format)", () => {
    test("formats millions with compact M suffix", () => {
      const result = formatCreditsCompact(1_000_000);
      expect(result).toBe("1M cr");
    });

    test("formats 2.5M correctly", () => {
      const result = formatCreditsCompact(2_500_000);
      expect(result).toBe("2.5M cr");
    });

    test("formats thousands with compact K suffix", () => {
      const result = formatCreditsCompact(1_000);
      expect(result).toBe("1K cr");
    });

    test("handles small numbers", () => {
      const result = formatCreditsCompact(999);
      expect(result).toBe("999 cr");
    });

    test("returns — for null", () => {
      const result = formatCreditsCompact(null);
      expect(result).toBe("—");
    });
  });

  describe("relativeTime (canonical, from @/lib/time)", () => {
    ...
    test("returns correct format for past times", () => {
      ...
      expect(result).toMatch(/^\d+[mhd] ago$|^just now$|\d+s ago/);
    });
  });
});
